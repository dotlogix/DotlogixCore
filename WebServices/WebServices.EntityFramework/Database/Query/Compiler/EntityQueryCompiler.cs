...
public class EntityQueryCompiler : QueryCompiler, IEntityQueryCompiler {
    ...
    private readonly IEvaluatableExpressionFilter _expressionFilter;
    ...
    private async Task<TResult> OnExecuteQueryAsync<TResult>(QueryContext ctx, Expression query, Func<QueryContext, Task<TResult>> executeFunc) {
        ...
        if(resultEventArgs.HasResult == false) {
            try {
                ...
                if(resultEventArgs.Exception is not null) {
                    throw;
                }
            }
        }
    ...
    public override Expression ExtractParameters(Expression query, IParameterValues parameterValues, IDiagnosticsLogger<DbLoggerCategory.Query> logger, bool parameterize = true, bool generateContextAccessors = false) {
        var visitor = new EntityQueryParameterExtractingExpressionVisitor(_rewriter, _expressionFilter, parameterValues, _contextType, _model, logger, parameterize, generateContextAccessors);
