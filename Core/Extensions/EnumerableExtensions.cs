...
namespace DotLogix.Core.Extensions {
	...
	public static class EnumerableExtensions {
		...
		/// <returns></returns>
		public static IEnumerable<T> Enumerate<T>(this T initialValue, Func<T, T> selectNextFunc, Func<T, bool> hasNextFunc,
		...
		/// <param name="yieldInitial">A flag if the initial value should be yield or skipped</param>
		/// <returns></returns>
		public static IEnumerable<T> EnumerateUntil<T>(this T initialValue, Func<T, T> selectNextFunc,
			Func<T, bool> conditionFunc, bool yieldInitial = false) {
			if (yieldInitial)
				yield return initialValue;
		...
		/// <returns></returns>
		public static T[] CreateArray<T>(this T value, int count = 1) { return Initialize(new T[count], value); }
		...
		/// <returns></returns>
		public static List<T> CreateList<T>(this T value, int count = 1) { return CreateArray(value, count).ToList(); }
		...
		public static IEnumerable<TTarget> SelectWith<TSource, TTarget, TWith>(this IEnumerable<TSource> source,
			Func<TSource, TWith, TTarget> selector, TWith with) {
		...
		/// <returns></returns>
		public static DiffEnumerable<T> Diff<T>(this IEnumerable<T> left, IEnumerable<T> right,
		...
		/// <param name="comparer">The comparer used to check equality</param>
		/// <returns></returns>
		public static DiffEnumerable<T, T> Diff<T, TKey>(this IEnumerable<T> left, IEnumerable<T> right,
			Func<T, TKey> keySelector, IEqualityComparer<TKey> comparer = null) where TKey : IComparable {
		...
		public static DiffEnumerable<TLeft, TRight> Diff<TLeft, TRight, TKey>(this IEnumerable<TLeft> left,
			Func<TLeft, TKey> leftKeySelector, IEnumerable<TRight> right, Func<TRight, TKey> rightKeySelector,
		...
		/// <param name="mode"></param>
		/// <returns></returns>
		public static IEnumerable<T> Combine<T>(this IEnumerable<T> enumerable, IEnumerable<T> otherEnumerable,
			CombineMode mode) {
			switch (mode) {
				case CombineMode.Sequential:
		...
		public static Hierarchy<TKey, TValue> ToHierarchy<TKey, TValue>(this IEnumerable<TValue> items,
			Func<TValue, TKey> keySelector, Func<TValue, TKey> parentKeySelector) where TKey : IComparable {
