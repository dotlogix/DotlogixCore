...
namespace DotLogix.Infrastructure.Repositories {
	/// <summary>
    ...
    public interface IRepository<TEntity> : IRepository where TEntity : class, new() {
        ...
        /// </summary>
        Task<ICollection<TEntity>> WhereAsync(IQueryModifier<TEntity, TEntity> filter, CancellationToken cancellationToken = default);
	...
	public interface IRepository<in TKey, TEntity> : IRepository<TEntity> where TEntity : class, new() {
		...
		/// </summary>
		Task<ICollection<TEntity>> GetRangeAsync(IEnumerable<TKey> keys, CancellationToken cancellationToken = default);
    }
}
