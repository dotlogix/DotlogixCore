...
namespace DotLogix.WebServices.Testing.NUnit.Json
{
    public static class JsonDiff
    {
        ...
        private static JsonDiffResult DiffRecursive(JToken expected, JToken actual, JsonDiffOptions options)
        {
            ...
            switch (expected.Type)
            {
                case JTokenType.Object:
                    return DiffObjectRecursive((JObject) expected, (JObject) actual, options);
                case JTokenType.Array:
                    return DiffArrayRecursive((JArray) expected, (JArray) actual, options);
                default:
                    return DiffValueRecursive((JValue) expected, (JValue) actual, options);
            }
        }
        ...
        private static JsonDiffResult DiffObjectRecursive(JObject expected, JObject actual, JsonDiffOptions options)
        {
            ...
            if (options.PropertyWhitelist != null && options.PropertyWhitelist.Count > 0)
            {
                expectedOnly.IntersectWith(options.PropertyWhitelist);
                actualOnly.IntersectWith(options.PropertyWhitelist);
            }

            if (options.PropertyBlacklist != null && options.PropertyBlacklist.Count > 0)
            {
        ...
        private static JToken ToJToken(object expected, JsonSerializer serializer)
        {
            switch (expected)
            {
                ...
                case JToken jToken:
                    return jToken;
                default:
                    return JToken.FromObject(expected, serializer);
            }
        }
    }
}